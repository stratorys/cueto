...
// pure engine must not import the workspace (persistence) or content (source
// formats) concerns. A violation means a disk or source-format dependency leaked
// into evaluation, breaking the transports -> services -> domain layering.
func TestEvaluationDoesNotImportSiblingConcerns(t *testing.T) {
	...
	forbidden := map[string]bool{
		"github.com/stratorys/cueto/backend/internal/workspace": true,
