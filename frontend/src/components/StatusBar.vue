...
// needs no announcement, so there is no "valid" badge - a clean bar is the signal.
defineProps<{
  saveState: SaveState;
  problemCount: number;
...
<template>
  <div class="flex h-6 flex-none items-center justify-between border-t border-slate-800 bg-slate-900 font-mono text-[11px] text-slate-400">
    <div class="flex h-full items-center">
      <button
        v-if="problemCount"
        :class="item"
        class="text-red-400 hover:bg-slate-800"
      ...
      <span v-else :class="item" class="text-slate-500">
        <Check class="h-3 w-3" />
        no problems
      </span>
    ...
    <div class="flex h-full items-center">
      ...
      >
        <Lightbulb class="h-3 w-3" :class="showHints ? 'text-amber-400' : 'text-slate-500'" />
        Hints
      </button>
